...
module.exports = (database = '', watcher = '', options = {}) => {
	...
	const {host, user, password, port, mysqldump} = options;
	if (!mysqldump) {
	...
	const createRestorationMap = async (tables) => {
		...
		return await Promise.reduce(tables, (acc, table) => {
			const command = `${mysqldump} -h ${host} -P ${port} -u ${user} --password=${password} --skip-comments --add-drop-table --compact ${database} ${table}`;
	...
	const watch = async () => {
		...
		const restorationMap = await createRestorationMap(tablesToWatch);
